...
def _create_media_item_from_dict(doc_id: str, raw_item_data: dict) -> MediaItem:
    ...
    media_item = MediaItem(
        ...
        dataset_name=raw_item_data.get("dataset_name"),
...
def get_media_for_page(
    ...
    filter_by_dataset: Optional[str] = None,  # New parameter
) -> List[MediaItem]:
    ...
    try:
        ...
        for doc in all_docs:
            ...
            # Additional client-side check for dataset name just in case
            if filter_by_dataset and raw_item_data.get("dataset_name") != filter_by_dataset:
                continue
...
def get_media_for_page_optimized(
    ...
    # This optimization is complex to update safely without testing composite indexes.
    # Leaving it mostly as-is but ensuring we can call it if needed.
    # For now, Cartoon Pro will rely on get_media_for_page or specific queries.
    return [], None
...
def get_media_for_chooser(
    ...
    try:
        ...
        # start_slice is 0 because we've already done limit() on queries, 
        # but since we merge and sort, we might have up to 2*page_size items unless distinct.
        # But for 'limit(page_size)' on both query1 and query2, we might have 2*page_size.
        # We just take the first page_size items.
...
def save_dataset(name: str, description: Optional[str] = None):
    """Saves a new dataset metadata to Firestore."""
    if not name:
        return
    
    try:
        # potential collection name collision check?
        # Using a dedicated collection for datasets
        doc_ref = db.collection("genmedia_datasets").document(name)
        doc_ref.set({
            "name": name,
        ...
        logger.info(f"Dataset '{name}' saved/updated.")
    except Exception as e:
...
def get_all_datasets() -> List[dict]:
    ...
    try:
        docs = db.collection("genmedia_datasets").order_by("name").stream()
